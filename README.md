...
Read from `.env` locally, or `.env.prod` in Docker.
...
docker compose pull && docker compose up -d --force-recreate   # recreate so an edited .env.prod is re-read
